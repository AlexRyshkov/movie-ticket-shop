...
function OrderProvider({ children }: Props) {
  ...
  const dispatchWithMiddleware = useCallback(
    (action: Action) => {
      if (router.pathname === "/order" && action.type === "removeTicket") {
        if (
          order.filter(
            (item) => item.movieId === action.payload && item.ticketCount === 1
          ).length === 1
        ) {
  ...
  return (
    <OrderContext.Provider value={{ order, dispatch: dispatchWithMiddleware }}>
      {showSubmitModal && (
        <SubmitModal
